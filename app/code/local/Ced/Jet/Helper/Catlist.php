...
class Ced_Jet_Helper_Catlist extends Mage_Core_Helper_Abstract{
			...
			try{
				...
				foreach($taxonomy as $txt){
					if(number_format($txt[3],0,'','')!=0)
					{
						...
						$query = 'INSERT INTO '.$table.' (csv_cat_id, name,csv_parent_id,jet_tax_code,level,attribute_ids,jetattr_names) VALUES ("'.$csv_cat_id.'", "'.$txt[1].'","'.$csv_parent_id.'","'.$txt[5].'","'.$txt[4].'","'.$attribute_ids.'","'.$attributes_names.'")';
	...
	public function getattributesids($category_id=""){
			...
			try{
				$comma_seperated_ids ='';
				foreach($taxonomy as $txt){
					if($category_id==number_format($txt[1],0,'','')){
						$comma_seperated_ids = $comma_seperated_ids.','.number_format($txt[0],0,'','');
					}
				}
