...
class Amasty_SeoGoogleSitemap_Model_Sitemap extends Mage_Core_Model_Abstract
{
    ...
    public function generateXml()
    {
		...
		if (Mage::helper('core')->isModuleEnabled('Amasty_Shopby')
			|| Mage::helper('core')->isModuleEnabled('Amasty_Brands')) {
	...
	protected function _generateCategories()
	{
        ...
        foreach ($collection as $item) {
            if ('true' == (string)Mage::getConfig()->getNode('modules/Enterprise_UrlRewrite/active')) {
                $item['url'] = $item->getUrl();
            } else {
                $item['url'] = $storeUrl . $item->getUrl();
            }
            if (empty($item['url'])) {
                continue;
            }
	...
	protected function _generateProducts()
	{
		...
		$needUseGetProductUrl = false;
		$isUniqueProductUrl = Mage::helper('core')->isModuleEnabled('Amasty_SeoSingleUrl');
        $magentoVersion = Mage::getVersion();
        if (version_compare($magentoVersion, '1.8', '>=')){
            $needUseGetProductUrl = true;
        } elseif ($isUniqueProductUrl) {
            if (Mage::getStoreConfig('amseourl/general/product_url_type') != 0) {
                $needUseGetProductUrl = true;
            }
        }
        ...
        do {
            ...
            foreach ($collection as $item) {
				$productUrl = Mage::getBaseUrl('web') . $item->getUrlPath();;
				if ($needUseGetProductUrl) {
	...
	protected function _generateBrands()
	{
		...
		if (Mage::helper('core')->isModuleEnabled('Amasty_Brands')) {
			$attrCode = trim(Mage::getStoreConfig('ambrands/general/attribute'));
			$helper = Mage::helper('ambrands');
		} else {
		...
		foreach ($brandPages->getSource()->getAllOptions() as $option) {
			if ($option['value']) {
				$url = $helper->getOptionUrl($attrCode, $option['value']);
	...
	protected function _generateNavigation()
	{
		...
		/** @var Amasty_Shopby_Model_Mysql4_Page_Collection $navigationPages */
		$navigationPages = Mage::getModel('amshopby/page')->getCollection();
		$navigationPages->addFieldToFilter('url', array('neq' => ''));

		foreach ($navigationPages as $navigationPage) {
			$url = $navigationPage->getUrl();
