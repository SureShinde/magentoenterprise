...
class Magestore_Onestepcheckout_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function add_couponAction() {
        ...
        try {
            ...
            if (!$oCoupon->isObjectNew() || $this->getRequest()->getParam('remove') == '1'){
            ...
            if ($couponCode) {
                if ($couponCode == $quote->getCouponCode()) {
                    ...
                    Mage::getSingleton('checkout/session')->addSuccess(
                        $this->__('Gift Card "%s" was added.', Mage::helper('core')->escapeHtml($couponCode))
                    );
                }
            } else {
            ...
            $error = true;
            $message = $this->__('Cannot apply the coupon code or gift card.');
        }
        $result = array(
            'error' => $error,
            'message' => $message
