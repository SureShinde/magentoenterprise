...
class TBT_Milestone_Model_Rule extends Mage_Core_Model_Abstract
{
    ...
    public function trigger($customerId)
    {
        ...
        /*
         * If a transfer was created for this action,
         * save the rule log as it's reference.
         */
        $transfer = $action->getTransfer();
        if ($transfer && $transfer->getId()) {
            $reference = Mage::getModel('rewards/transfer_reference')
                ->loadReferenceByTransferId($transfer->getId());
            if ($reference && $reference->getId()) {
                $reference->setReferenceId($ruleLog->getId());
                $reference->save();
            }
        }
