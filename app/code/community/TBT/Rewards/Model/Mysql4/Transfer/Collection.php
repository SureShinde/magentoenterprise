...
class TBT_Rewards_Model_Mysql4_Transfer_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
    ...
    public function selectNonExpiryTransfers()
    {
        $this->addFieldToFilter(
            'reason_id',
            array('neq' => TBT_Rewards_Model_Transfer_Reason_PointsExpiry::REASON_TYPE_ID)
        );
    ...
    public function selectExpiryTransfers()
    {
        $this->addFieldToFilter(
            'reason_id',
            array('eq' => TBT_Rewards_Model_Transfer_Reason_PointsExpiry::REASON_TYPE_ID)
        );
    ...
    public function prepareMetricsPointsActivity($range, $type = 'earn')
    {
        ...
        $select->columns(array(
            ...
            'range' => new Zend_Db_Expr(
                "MAX(".$this->_getRangeExpression($range, 'creation_ts').")"
            )
        ));
        
        $select->group($this->_getRangeExpression($range, 'creation_ts'));

        $select->where("DATE_FORMAT(creation_ts, '%Y-%m-%d') >= '" . $dateRange['from']->toString('Y-MM-dd') . "'");
    ...
    public function getTotalPointsActivityByDate($startDate, $after = true)
    {
        ...
        if ($startDate) {
            $operand = ($after) ? '>=' : '<';
            $select->where("DATE_FORMAT(creation_ts, '%Y-%m-%d') ". $operand ." '" . $startDate . "'");
        }
