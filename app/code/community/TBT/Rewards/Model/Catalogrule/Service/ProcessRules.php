    ...
    extends Mage_Core_Model_Abstract
{
    ...
    protected function _refactorRedemption(&$persistentItem, $doSave = true)
    {
        ...
        $item->setBaseRowTotal(Mage::helper('rewards/price')->getReversedCurrencyPrice($rowTotal));
        $item->setBaseRowTotalInclTax(Mage::helper('rewards/price')->getReversedCurrencyPrice($rowTotalInclTax));
    ...
    protected function _updateRedemptionData($item, $doInclTax = true)
    {
        ...
        if ($item->hasCustomPrice()) {
            ...
            if ($this->_taxHelper->priceIncludesTax() && $item->getPriceInclTax()) {
                $productPrice = $item->getPriceInclTax() / (1 + $item->getTaxPercent() / 100);
            } else {
