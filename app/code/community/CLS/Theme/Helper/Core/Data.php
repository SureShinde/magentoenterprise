...
class CLS_Theme_Helper_Core_Data extends Mage_Core_Helper_Data
{
    protected $_eventPrefix = 'core_data';
            ...
            $beforeMergeCallback = null, $extensionsFilter = array())
    {
        try {
                ...
                if (!file_exists($targetFile)) {
                    ...
                    foreach ($srcFiles as $file) {
                        if (!file_exists($file) || @filemtime($file) > $targetMtime) {
                            if(strpos($file, 'http', 0) !== 0) { //external resources aren't interesting 
                                Mage::log('Can\'t find JS/CSS merge file: '.$file);
                            }
            ...
            if ($shouldMerge) {
                ...
                //event
                $paramsDataWrapper = new Varien_Object();
                $paramsDataWrapper->setSrcFiles($srcFiles)
                                  ->setTargetFile($targetFile)
                                  ->setMustMerge($mustMerge)
                                  ->setBeforeMergeCallback($beforeMergeCallback)
                                  ->setExtensionsFilter($extensionsFilter)
                                  ->setMergedData($data);
                
                Mage::dispatchEvent($this->_eventPrefix.'_merge_after',
                        array('params'=>$paramsDataWrapper)
                );
                
                if($paramsDataWrapper->getOrigData('merged_data') != $paramsDataWrapper->getMergedData()) {
                    $data = $paramsDataWrapper->getMergedData();
                }
                //end event
