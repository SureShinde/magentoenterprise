...
class Fishpig_Wordpress_Model_Resource_Post extends Fishpig_Wordpress_Model_Resource_Abstract
{
	...
	public function getParentTermsByPostId($postId, $taxonomy = 'category')
	{
		...
		if (Mage::helper('wordpress/plugin')->isEnabled('wordpress-seo/wp-seo.php')) {
			$select->order('_term.name ASC');
			
			$tempPostModel = Mage::getModel('wordpress/post')->setId($postId);

			if ($categoryId = $tempPostModel->getMetaValue('_yoast_wpseo_primary_category')) {
				$select->where('_term.term_id=?', $categoryId);
			}
		}
		else {
			$select->order('_term.term_id ASC');
		}
