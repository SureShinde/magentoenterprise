...
Checkout.prototype = {
    ...
    reloadStep: function(prevStep) {
        var updater = new Ajax.Updater(prevStep + '-progress-opcheckout', this.progressUrl, {
            ...
            onComplete: function(){
                this.checkout.updateCheckoutTotal(prevStep);
    ...
    updateCheckoutTotal: function(prevStep) {
        if(typeof prevStep != 'undefined') {
            var totalEl = jQuery('#total-progress-opcheckout .price');

            var newTotal = jQuery('#' + prevStep + '-progress-opcheckout dd.progress-total .price');

            totalEl.text(newTotal.text());
        }
    },
